...
class protoAugSSL:
    ...
    def implicit_aug(self, features, y, labels, ratio):
        N = features.size(0)
        C = self.numclass
        A = features.size(1)
    ...
    def _compute_loss(self, imgs, target, old_class=0):
        ...
        if self.old_model is None:
            ...
            if self.args.protoAug_mode == 'E':
                for _ in range(int(self.args.old_number * self.args.batch_size)):
            ...
            else:  # self.args.protoAug_mode == 'I':
                for _ in range(int(self.args.old_number * self.args.batch_size)):
                    np.random.shuffle(index)
                    temp = self.prototype[index[0]]
                    proto_aug.append(temp)
                    aug_label.append(self.class_label[index[0]])
                    proto_aug_label.append(4 * self.class_label[index[0]])

                proto_aug = torch.from_numpy(np.float32(np.asarray(proto_aug))).float().to(self.device)
                proto_aug_label = torch.from_numpy(np.asarray(proto_aug_label)).to(self.device)
                aug_label = torch.from_numpy(np.asarray(aug_label)).to(self.device)
                soft_feat_aug = self.model.fc(proto_aug)
                soft_feat = soft_feat_aug[:, ::4]

                implicit_proto_aug = self.implicit_aug(proto_aug, soft_feat, aug_label, self.args.ratio)
                soft_feat_aug[:, ::4] = implicit_proto_aug
                loss_protoAug = nn.CrossEntropyLoss()(soft_feat_aug / self.args.temp, proto_aug_label)
    ...
    def protoSave(self, model, loader, current_task):
        ...
        for item in labels_set:
            ...
            if current_task == 0:
                radius.append(np.trace(cov_class) / feature_dim)
            if self.args.protoAug_mode == 'I':
